...
Component({
  ...
  methods: {
    ...
    drawImage (params) {
      this.ctx.save()
      const { url, top = 0, left = 0, width = 0, height = 0, borderRadius = 0 ,rotate,angle,scale,num,prop,x,y,jiaowid,jiaohei} = params
      if (borderRadius) {
        this.ctx.beginPath()
        ...
        if(angle!=undefined){
          this.ctx.translate(this.data.width* (1 - num) / 2, 10);
          console.log(this.data.width)
          this.ctx.beginPath();
          this.ctx.translate(x* prop+jiaowid,  y * prop-jiaohei); //圆心坐标
          this.ctx.rotate(angle * Math.PI / 180); // 旋转值
          this.ctx.translate(-(width * scale * prop / 2), -(height * scale * prop / 2))
          this.ctx.drawImage(url, 0, 0, width * scale * prop, height * scale * prop);
